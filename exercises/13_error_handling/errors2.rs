...
fn total_cost(item_quantity: &str) -> Result<i32, ParseIntError> {
    ...
    // the short way, use the ? operator to propagate the error since it contains it's own ParseIntError
    let qty = item_quantity.parse::<i32>()?;
    
    Ok(qty * cost_per_item + processing_fee)

    // the long way..
    //  // remove this assignment  let qty = item_quantity.parse::<i32>();
    //   
    // and just match directly on the item_quantity returning the result
